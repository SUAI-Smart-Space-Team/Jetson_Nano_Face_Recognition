...
if __name__ == '__main__':
    ...
    ###
    test = False    # True -- data_from_cameras.json, False -- real_cameras
    n_cam = 1   # 0 -- bot, 1 -- top
    ###
    cam = 'bot' if n_cam == 0 else 'top'
    camera = cameras[n_cam]
    ...
    if test:
        with open('data_from_cameras.json') as json_file:
            data_from_cameras = json.load(json_file)

        for k in range(2):
            h = data_from_cameras[k]['H']
            w = data_from_cameras[k]['W']
            j = data_from_cameras[k]['j']
            i = data_from_cameras[k]['i']
            r = data_from_cameras[k]['r']
        ...
        for num_exp in range(1, 4):
            with open('json_data_from_cameras/data_' + cam + '_' + str(num_exp) + '.json') as json_file:
                data_from_cameras = json.load(json_file)

            w = data_from_cameras['weight']
            h = data_from_cameras['height']
            i = (data_from_cameras['people'][0]['bbox'][0] + data_from_cameras['people'][0]['bbox'][1]) / 2     # x
            j = (data_from_cameras['people'][0]['bbox'][2] + data_from_cameras['people'][0]['bbox'][3]) / 2     # y
            r = data_from_cameras['people'][0]['depth']

            person_point_real = get_coordinates(camera=camera, h=h, w=w, j=j, i=i, r=r)
            ax.plot(gt[num_exp - 1].x, gt[num_exp - 1].y, gt[num_exp - 1].z, 'co', markersize=num_exp+2)
            ax.plot(person_point_real.x, person_point_real.y, person_point_real.z, 'bo', markersize=num_exp+2)
